 ...
 * \see IParser
 */
inline IParser* createParser(nvinfer1::INetworkDefinition& network, nvinfer1::ILogger& logger, const std::vector<nvinfer1::Dims>& input_dims={}, int explicit_batch_size=1)
{
    return static_cast<IParser*>(createNvOnnxParser_INTERNAL(&network, &logger, NV_ONNX_PARSER_VERSION, input_dims, explicit_batch_size));
}
