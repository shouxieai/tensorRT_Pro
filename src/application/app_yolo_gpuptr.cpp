...
static void inference_and_performance(int deviceid, const string& engine_file, TRT::Mode mode, YoloGPUPtr::Type type, const string& model_name){

    auto engine = YoloGPUPtr::create_infer(
    ...
    vector<YoloGPUPtr::Image> images_gpu;
    vector<cv::Mat> images;
    for(int i = 0; i < files.size(); ++i){
        auto image           = cv::imread(files[i]);
        size_t image_size    = image.rows * image.cols * 3;
        uint8_t* pimage_data = nullptr;
        auto stream = (cudaStream_t)engine->stream();

        checkCudaRuntime(cudaMalloc(&pimage_data, image_size));
        checkCudaRuntime(cudaMemcpyAsync(pimage_data, image.data, image_size, cudaMemcpyHostToDevice, stream));
        checkCudaRuntime(cudaStreamSynchronize(stream));
        images_gpu.emplace_back(YoloGPUPtr::Image(pimage_data, image.cols, image.rows, deviceid, stream, YoloGPUPtr::ImageType::GPUBGR));
        images.emplace_back(image);
    }

    // warmup
    vector<shared_future<YoloGPUPtr::BoxArray>> boxes_array;
    for(int i = 0; i < 10; ++i)
        boxes_array = engine->commits(images_gpu);
    ...
    auto type_name = YoloGPUPtr::type_name(type);
...
static void test(YoloGPUPtr::Type type, TRT::Mode mode, const string& model){
...
int app_yolo_gpuptr(){
    ...
    test(YoloGPUPtr::Type::V5, TRT::Mode::FP32, "yolov5s");
