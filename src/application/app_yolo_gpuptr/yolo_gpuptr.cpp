...
namespace YoloGPUPtr{
    ...
    public:
        ...
        virtual bool preprocess(Job& job, const Image& image) override{
            ...
            // device changed
            uint8_t* pimage_device = image.device_data;
            if(tensor->device() != image.device_id){
                size_t image_data_size = image.get_data_size();
            ...
            if(image.type == ImageType::GPUYUVNV12){
                CUDAKernel::convert_nv12_to_bgr_invoke(
                    pimage_device, pimage_device + image.width * image.height,
            ...
            CUDAKernel::warp_affine_bilinear_and_normalize_plane(
                image_device,         image.get_width() * 3,    image.get_width(),       image.get_height(), 
                tensor->gpu<float>(), input_width_,             input_height_, 
                affine_matrix_device, 114, 
                normalize_, preprocess_stream
            );

            if(pimage_device != image.device_data){
                checkCudaRuntime(cudaStreamSynchronize(preprocess_stream));
                checkCudaRuntime(cudaFree(pimage_device));
            }
            return true;
        }

        virtual vector<shared_future<BoxArray>> commits(const vector<Image>& images) override{
